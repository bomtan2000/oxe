...
export class SSOUserProfileService extends ApiSSO {
  ...
  private ssoUserModule: string = 'User';

  constructor(private execCallApi: ExecCallApi, private _httpClient: ApplicationHttpClient) {
    super();
  }
  ...
  async getUserInfoAsync(id: number) {
    this.isAuthenSSO = true;
    const endPoint = `${this.ssoUserModule}/${id}`;
